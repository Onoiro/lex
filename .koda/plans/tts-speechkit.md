...
### Issue 1: Рассинхрон озвучивания в режиме Повтор (Review.tsx)

**Статус:** открыт

**Симптомы (со слов пользователя):**
- Первое слово при показе озвучивается корректно
- Далее озвучивание отстаёт: когда на экране уже следующее слово, звучит перевод предыдущего слова, а затем «перевод перевода»
- Слово, которое сейчас отображается на экране, не озвучивается вообще
- При нажатии кнопки «Остановить» озвучивание продолжается, хотя слова уже не показываются

**Анализ:**
- `synthesizeSpeech` в `ttsApi.ts` работает асинхронно: fetch + `Audio.play()` занимают время
- Review переключает слова быстро (авто-next через 3 сек, ручной next мгновенно)
- Несколько `synthesizeSpeech` вызовов летят параллельно и разрешаются в произвольном порядке
- Нет механизма отмены: новое воспроизведение не останавливает/инвалидирует предыдущее
- `stopTts()` не вызывается при остановке/паузе/переходе

**Попытки исправления (не сработали):**
1. Добавление `currentAudio.pause()` перед новым воспроизведением — не помогло, т.к. проблема в порядке разрешения fetch-запросов, а не в накладывающемся аудио
2. Генерационный токен (`generationToken`) + `stopTts()` при переходе/паузе/размонтировании — не помогло, проблема воспроизводится

**Что нужно сделать:**
- Тщательно проанализировать последовательность вызовов `playTts` в Review.tsx: `showNextWord`, `handleAnswer`, `handleStart`, кнопка «Показать перевод»
- Возможно, проблема в том, что `playTts` вызывается для `currentViewRef.current`, который ещё не обновился после `setCurrentView`
- Рассмотреть возможность: не вызывать `playTts` в `handleAnswer` для перевода, а только в `showNextWord` для слова
- Добавить логирование/отладку для точного понимания последовательности вызовов
- Убедиться, что `stopTts()` вызывается во всех точках перехода между словами и выхода из тренировки
